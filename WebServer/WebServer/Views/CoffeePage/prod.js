...
// ========== Отзывы ==========
// Добавьте эту функцию в prod.js
function initReviews() {
    const reviewsContainer = document.getElementById('reviewsContainer');
    const reviewsLoading = document.getElementById('reviewsLoading');
    const reviewForm = document.getElementById('reviewForm');

    if (!reviewsContainer || !reviewsLoading || !reviewForm) {
        console.log('Элементы отзывов не найдены');
        return;
    }

    // Получаем catalog_id из глобальной переменной
    const catalogId = window.productCatalogId;
    console.log('initReviews: catalogId =', catalogId);

    if (!catalogId) {
        console.error('catalogId не найден!');
        reviewsContainer.innerHTML = '<p class="error-message">⚠️ Ошибка: ID продукта не найден</p>';
        return;
    }
    ...
    reviewForm.addEventListener('submit', async function (e) {
        ...
        console.log('Отправка отзыва:', { name, text, stars, catalogId });
    ...
    async function loadReviewsFromAPI(catalogId) {
        ...
        try {
            console.log(`Загрузка отзывов для catalogId=${catalogId}`);
            ...
            reviewsContainer.innerHTML = '<p class="error-message">❌ Ошибка при загрузке отзывов</p>';
        } finally {
            hideLoading();
        }
    }
...
document.addEventListener('DOMContentLoaded', function () {
    console.log('DOM загружен, productCatalogId =', window.productCatalogId);

    initProductPage();
    initFavoriteButton();
    initImageZoom();
    // initReviews уже вызывается в initProductPage, но убедимся
    if (typeof initReviews === 'function') {
        // Если initReviews еще не вызвана, вызываем
        setTimeout(() => {
